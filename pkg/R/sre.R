sre <- function (Response = NULL, Explanatory = NULL, NewData = NULL, 
    Quant = 0.025) 
{
...
            Pred <- as.data.frame(matrix(0, nr = dim(NewData)[1], 
                nc = dim(Response)[2], dimnames = list(seq(dim(NewData)[1]), 
                  colnames(Response))))
                  
...
            if (class(NewData)[1] == "RasterStack") {
                TF <- subset(NewData,1)
                TF <- TF >= TF@data@min    ## create a mask with 1
            
