...
function(initial, projections, plothist=T, distrib=T, cvsn=T, groups=NULL, resolution=5){
    ...
    projections <- projections[,which(projections[1,]!="NA")]
  	pres <- c() ; for(i in 1:length(initial)) if(initial[i] == 1) pres <- c(pres,i)
  	...
  	p <- hist(area, breaks = seq(a,b,resolution), plot=F) ; p$density <- p$counts / sum(p$counts)
  	if(distrib){
    		a.s <- sort(area)
  	...
  	if(cvsn){
    		area2 <- (apply(projections[pres,],2,sum) / sum(initial==1) -1) * 100
    		area3 <- area - area2
  	}
    if(!is.null(groups)){
    		x11()
            ...
            lv <- levels(as.factor(as.matrix(groups[pa,])))
      			color.samp[[pa]] <- colors()[sample(c(90,417,552,616,382,11,150,468,28,31,420,476,341),length(lv))]
      			g <- hist(area, breaks = seq(a,b,resolution), plot=F)
      			fac <- (max(g$counts) / sum(g$counts)) / max(g$density)
      			g$density <- g$density * fac
      
      			plot(g, freq=F, border='grey80', main=paste('groups',pa,sep=" "), xlab="Species range change (%)", ylab="Event   occurence   probability")
    			  ...
    			  leg <- list() ; for(j in 1:length(levels(lv))) leg[[j]] <- levels(lv)[j]
    			  legend(-100, 0.1, legend=leg, bty='n',fill=color.samp[[pa]])
  		  }
    }
    if(cvsn){
    x11(); par(mfrow=c(1,nrow(groups)))
        ...
        for(i in 1:nrow(groups)){
      			...
      			abline(0,-1, col='grey80') ; abline(100,-1, col='grey80')
    ...
    if(plothist){
    		x11() ; par(mfrow=c(1,1))
    		if(!distrib){
      			plot(p,freq=F, col="palegoldenrod",xlim=c(a,b), ylim=c(0,max(p$density)*1.2),
      			main="Probability density function", xlab="Species range change (%)", ylab="Event   occurence   probability")
    		} else{
      			...
      			par(new=T); plot(p,freq=F, col="white",xlim=c(a,b), ylim=c(0,max(p$density)*1.2),
      			main="Probability density function", xlab="Species range change (%)", ylab="Event   occurence   probability")
    		}
  	}
}
