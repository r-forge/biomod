...
function(ANN=TRUE,CTA=TRUE,GAM=TRUE,GBM=TRUE,GLM=TRUE,MARS=TRUE,FDA=TRUE,RF=TRUE,SRE=TRUE, Proj.name, weight.method, decay=1.6, PCA.median=TRUE, binary=TRUE, bin.method='Roc', Test=FALSE, repetition.models=TRUE, final.model.out=FALSE)
{
    ...
    for(i in 1:Biomod.material$NbSpecies){
        ...
        for(ii in 1:nbrun){
            ...
            for(j in 1:NbPA){ #Loop through PAs replicates
                for(k in 1:nbrep){ #Loop through Eval replicates (nbrep set to 1 if no reps wanted -> 1 is PA 100% calib model)
                    ...
                    if(sum(RUNens.choice)>1){                                                                            #if more than 1 model is wanted for ensemble forecating
                        ...
                        STACK.m <- stack(STACK.m, round(mean(dropLayer(STKcons, as.numeric(which(RUNens.choice!=ens.choice))))))
                        STACK.med <- stack(STACK.med, round(mean(dropLayer(STKcons, as.numeric(which(RUNens.choice!=ens.choice))))))                                          #median not functioning
                        ...
                        if(Biomod.material$algo[RUNens.choice] != 'SRE'){  
                            ...
                            for(jj in 1:3){ if(Biomod.material$evaluation.choice[Th[jj]]){
                                ...
                                if(Th[jj]=='Roc')   STACK.R <- stack(STACK.R, STKcons > thresh)   
                                if(Th[jj]=='Kappa') STACK.K <- stack(STACK.K, STKcons > thresh) 
                                if(Th[jj]=='TSS')   STACK.T <- stack(STACK.T, STKcons > thresh) 
            ...
            ths[[3]] <- ths[[1]]                                                                          #attribute the mean thresholds to median data (no such relevant thing as a median th)
            rownames(out[["weights"]]) <- colnames(out[["thresholds"]]) <- gnames
            if(sum(RUNens.choice) <= 1) out[["weights"]] <- NA
            list.out[[i]] <- out 
            ...
            if(ii==1){ #normal ensFor run --> saving on disk, transform in binary, total consensus
                ...
                if(final.model.out){
                    ...
                    STACK.m <- dropLayer(STACK.m, c(which(PaLayers)))
                    STACK.w <- dropLayer(STACK.w, c(which(PaLayers)))
                    STACK.med <- dropLayer(STACK.med, c(which(PaLayers)))
                }
                ...
                STACK.all <- stack(STACK.m, STACK.w, STACK.med)
                if((nbrep*NbPA) != 1) STACK.consTot <- stack(round(mean(STACK.m)), round(mean(STACK.w)), round(mean(STACK.med)))
                ...
                if((nbrep*NbPA) != 1) for(j in 1:3) STACK.consTot.bin <- stack(STACK.consTot.bin, STACK.consTot@layers[[j]] > mean(ths[[j]]))
                ...
                if(Biomod.material$evaluation.choice['Roc']){                
                    if(final.model.out) STACK.R <- dropLayer(STACK.R, c(which(PaLayers)))
                ...
                if(Biomod.material$evaluation.choice['Kappa']){
                    if(final.model.out) STACK.K <- dropLayer(STACK.K, c(which(PaLayers)))
                ...
                if(Biomod.material$evaluation.choice['TSS']){
                    if(final.model.out) STACK.T <- dropLayer(STACK.T, c(which(PaLayers)))
                ...
                #Adapt layerNames to data
                if(final.model.out) LNa <- LNa[rep(!PaLayers, (length(LNa)/length(gnames)))]
                                  
                #if binary=T, we transform the ensemble forecasting values into binary ones using the thresholds stored in ths (automatically done for final consensus)
                #first step (ths.lin) -> constitute a vector with all thresholds in associated to the stack in the same order (for looping the binary conversion)
                if(binary){ 
                    ths.lin <- c()
                    for(j in 1:6){
                        if(j>3){ if(Biomod.material$evaluation.choice[Th[j-3]]) ths.lin <- c(ths.lin, ths[[j]])                 
                        } else ths.lin <- c(ths.lin, ths[[j]])
                    }
                    STACK.all.bin <- stack()
                ...
                #STACK.consTot
                assign(paste("Total_consensus_", Biomod.material$species.names[i], "_", Proj.name, ".raster", sep=""), STACK.consTot.bin) 
