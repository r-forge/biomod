...
# setMethod('.Models.prepare.data', signature(data='BIOMOD.formated.data.indep'),
#           function(data, NbRunEval, DataSplit, Yweights=NULL){
#             list.out <- callNextMethod(data, NbRunEval, DataSplit, Yweights)
#             list.out[[1]]$indep.xy <- data@indep.coord
#             list.out[[1]]$indep.dataBM <- data.frame(cbind(data@indep.data.species,
#                                                            data@indep.data.env.var))
# 
#             return(list.out)
#           })
# 
# setMethod('.Models.prepare.data', signature(data='BIOMOD.formated.data.PA.indep'),
#           function(data, NbRunEval, DataSplit, Yweights=NULL){
#             list.out <- callNextMethod(data, NbRunEval, DataSplit, Yweights)
#             for(i in 1:length(list.out)){
#               list.out[[i]]$indep.xy <- data@indep.coord
#               list.out[[i]]$indep.dataBM <- data.frame(cbind(data@indep.data.species,
#                                                             data@indep.data.env.var))    
#             }
#             return(list.out)
#           })
